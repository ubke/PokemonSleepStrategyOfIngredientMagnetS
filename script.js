...
function updateDisplay() {
    ...
    // ※今回は「作れる料理」リストなので、部分一致も含めて出しつつ、
    // 数字ロジック（リーチ数）は別途計算する。
    
    // 【新ロジック】各食材の「リーチ数」を計算
    // 「未選択のこの食材を選べば、あと何個の料理が作れるようになるか？」
    const potentialCounts = {};
    allIngredients.forEach(ing => potentialCounts[ing] = 0);

    allRecipes.forEach(recipe => {
        // 現在選択中のカテゴリ以外はカウントしない（ユーザーの関心外のため）
        ...
        // もし「足りない食材が1つだけ」なら、その食材は「リーチ」状態
        if (missing.length === 1) {
    ...
    // (ここでは「カテゴリが合致」かつ「関連食材が選ばれている」ものをリストアップする既存ロジックを維持)
