...
const shortNameMap = {
    ...
    "リラックスカカオ": "カカオ",
    "あじわいキノコ": "キノコ",
    "ふといながねぎ": "ながねぎ", // ここを「ネギ」から「ながねぎ」に変更
    "ずっしりカボチャ": "カボチャ",
    "ワカクサコーン": "コーン",
    "つやつやアボカド": "アボカド",
    "めざましコーヒー": "コーヒー",
...
function init() {
    ...
    allIngredients.forEach(ing => {
        ...
        const displayName = shortNameMap[ing] || ing;
...
function updateDisplay() {
    ...
    allIngredients.forEach(ing => {
        ...
        if (selectedIngredients.has(ing)) {
            ...
            if (count > 0) {
                btn.innerHTML = `${icon} ${displayName} <span style="color:#e91e63; font-weight:bold; margin-left:4px;">(${count})</span>`;
            } else {
                btn.innerHTML = `${icon} ${displayName}`;
            }
        }
